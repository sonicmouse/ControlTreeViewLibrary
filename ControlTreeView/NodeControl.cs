namespace ControlTreeView
{
	...
	public class NodeControl : UserControl, INodeControl
	{
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			if (OwnerNode?.OwnerCTreeView is not null && OwnerNode.OwnerCTreeView.SelectionMode != CTreeViewSelectionMode.None)
			{
			...
			if (OwnerNode?.OwnerCTreeView is not null)
			{
				mouseDownPosition = OwnerNode.OwnerCTreeView.PointToClient(Cursor.Position);//mouseDownPosition = e.Location;
			}
		...
		private void StartDragging(object? sender, MouseEventArgs e)
		{
			if (OwnerNode?.OwnerCTreeView is not null)
			{
				var movePoint = OwnerNode.OwnerCTreeView.PointToClient(Cursor.Position);
		...
		private void NotDragging(object? sender, MouseEventArgs e)
		{
			this.MouseMove -= StartDragging;
			this.MouseUp -= NotDragging;
			if (unselectAfterMouseUp && OwnerNode is not null)
			{
			...
			if (unselectOtherAfterMouseUp)
			{
				if (OwnerNode?.OwnerCTreeView is not null)
				{
					var nodesToUnselect = new List<CTreeNode>(OwnerNode.OwnerCTreeView.SelectedNodes);
