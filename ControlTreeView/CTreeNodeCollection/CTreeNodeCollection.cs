namespace ControlTreeView
{
	...
	{
		private readonly INodeContainer _owner;
		...
		/// </summary>
		internal CTreeView? ParentCTreeView => _owner.OwnerCTreeView;

		private void BeginUpdateCTreeView()
		{
			if (ParentCTreeView is not null)
			{
				ParentCTreeView.BeginUpdate();
			}
		}
		...
		protected override void InsertItem(int index, CTreeNode item)
		{
			...
			else
			{
				...
				if (_owner is CTreeNode ownerNode)
				{
		...
		internal List<CTreeNode.Line> GetLines(
			Func<CTreeNodeCollection, CTreeNode.Line>? commonLineCalc,
			Func<CTreeNode, CTreeNode.Line>? childLineCalc)
		{
			var lines = new List<CTreeNode.Line>();
			if (commonLineCalc is not null && Count > 1) lines.Add(commonLineCalc(this));
			if(childLineCalc is not null)
			{
