namespace ControlTreeView
{
	public partial class CTreeView
	{
		...
		{
			if (drgevent.Data is not null && drgevent.Data.GetDataPresent(typeof(List<CTreeNode>)) /*&& DragAndDropMode != CTreeViewDragAndDropMode.Nothing*/)
			{
				var sourceNodes = drgevent.Data.GetData(typeof(List<CTreeNode>)) as List<CTreeNode>;
				...
				if (sourceNodes is not null && sourceNodes[0].OwnerCTreeView == this)
				{
		...
		protected override void OnDragDrop(DragEventArgs drgevent)
		{
			if (drgevent.Data is not null && drgevent.Data.GetDataPresent(typeof(List<CTreeNode>)) /*&& DragAndDropMode != CTreeViewDragAndDropMode.Nothing*/)
			{
				...
				if (sourceNodes is not null && sourceNodes[0].OwnerCTreeView == this && CheckValidDrop(sourceNodes))
				{
					BeginUpdate();
					...
					else if (DragTargetPosition.NodeBefore != null && !sourceNodes.Contains(DragTargetPosition.NodeBefore))
					{
						...
						if(DragTargetPosition.NodeBefore.Parent is not null)
						{
					...
					else if (DragTargetPosition.NodeAfter != null && !sourceNodes.Contains(DragTargetPosition.NodeAfter))
					{
						...
						if(DragTargetPosition.NodeAfter.Parent is not null)
						{
