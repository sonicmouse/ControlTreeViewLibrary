...
namespace ControlTreeView
{
	public partial class CTreeView : Panel, INodeContainer
	{
			...
			: base()
		{
			...
			IndentDepth = 30;
			IndentWidth = 10;
			selectionPen = new(Color.Black, 1.0F)
			{
				DashStyle = DashStyle.Dot
			};
			selectionBrush = new SolidBrush(SystemColors.Highlight);
			_LinesPen = new(Color.Black, 1.0F)
			{
				DashStyle = DashStyle.Dot
			};

			var ip = Assembly.GetExecutingAssembly().GetManifestResourceStream("ControlTreeView.Resources.plus.bmp");
			var im = Assembly.GetExecutingAssembly().GetManifestResourceStream("ControlTreeView.Resources.minus.bmp");
			if (ip is not null && im is not null)
			{
				PlusMinus = new CTreeViewPlusMinus(new Bitmap(ip), new Bitmap(im));
			}
