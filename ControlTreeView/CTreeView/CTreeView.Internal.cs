		...
		internal void Recalculate()
		{
			if (!SuspendUpdate)
			{
				...
				foreach (CTreeNode rootNode in Nodes)
				{
					...
					rootNode.Nodes.TraverseNodes(node => node.ParentNode?.IsExpanded ?? false, node =>
					{
						node.Visible = true;
					});
				}
				...
				switch (DrawStyle)
				{
					case CTreeViewDrawStyle.LinearTree:
						...
						var startLocation = new Point(Padding.Left + 3, Padding.Top + 3);
						...
						parentLineCalc = new Func<CTreeNode, CTreeNode.Line>(parent => new CTreeNode.Line(
							...
							new Point(parent.Location.X + 5, parent.FirstNode?.Location.Y + parent.FirstNode?.Bounds.Height / 2 ?? 0)));
				...
				this.Nodes.TraverseNodes(node =>
				{
					...
					var controlLocation = node.Location;
					controlLocation.Offset(AutoScrollPosition);
					if (node.Control is NodeControl control) controlLocation.Offset(-control.Area.X, -control.Area.Y);
